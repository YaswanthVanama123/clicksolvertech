...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: 'Inventory & Sales Mobile App',
    client: 'React Native field app for inventory & sales',
    category: 'Mobile',
    tags: ['React Native', 'Node.js', 'Express', 'PostgreSQL', 'REST APIs'],
    desc: 'Mobile companion to the Enviromaster web platform. Field teams manage stock checkouts, run inventory operations, and capture sales entries on the go — with offline-friendly forms and a touch-first interface.',
    impact: 'Field-ready · Inventory + sales · Offline-tolerant · Same backend as web',
    icon: Smartphone,
    ...
    details: {
      overview: [
        "Field teams shouldn't have to wait until they're back at a desktop to update inventory or log a sale. This mobile app extends the Enviromaster web platform into a touch-first field tool — running stock checkouts, inventory adjustments, delivery confirmations, and sales entries directly on-site.",
        'The mobile experience shares the same backend and data model as the web platform, so anything captured in the field — inventory or sales — is immediately visible to operations on the web dashboards.',
      ],
      role: [
        'Designed and built the React Native app for both inventory operations and sales workflows in the field.',
      ...
      features: [
        { title: 'Mobile Inventory Operations', desc: 'Stock checkouts, inventory adjustments, and delivery confirmations from the warehouse floor or the field — no desktop required.' },
        { title: 'Field Sales Entry', desc: 'Reps capture sales directly on-site — no double entry, no waiting for desktop access.' },
        { title: 'Real-Time Inventory Sync', desc: 'Mobile inventory updates flow straight into the web dashboards so the operations team sees stock changes the moment they happen.' },
        { title: 'Offline-Friendly Forms', desc: 'Forms gracefully handle no-signal moments — data is held locally and synced as soon as connectivity returns.' },
      ...
      challenges: [
        ...
        { challenge: 'Keeping mobile and web inventory data in lockstep without duplicating business logic across two codebases.', solution: 'Reused the existing Enviromaster APIs and PostgreSQL schema as the single source of truth — the mobile app is a new client on top, not a parallel system.' },
      ],
      outcomes: [
        'Brought both inventory operations and sales workflows to where the work actually happens — out in the field.',
